...
// Exceptional leaves are one-off events — once their end date has passed, drop them
// so the "إجازات استثنائية" list doesn't accumulate stale entries. Payment-group
// membership is untouched (it's meant to persist).
function pruneExpiredExceptionalLeaves(soldiersList, todayKey) {
  return soldiersList.map(s => {
    const leaves = (s.leaves || []).filter(l => l.endDate >= todayKey);
    if (leaves.length === (s.leaves || []).length) return s;
    return {
      ...s,
      leaves
    };
...
// ---------- Reusable bottom sheet: scroll-lock, swipe-down-to-close, animations ----------
function BottomSheet({
  ...
  const sheetInnerRef = useRef(null);
  useEffect(() => {
  ...
  const requestClose = useCallback(() => {
    // cancel any in-progress entrance animation so it can't fight the imperative
    // close transition below over the same `transform` property.
    setEntered(true);
  ...
  // Drives the actual close motion once `closing` flips true. Rather than a fixed
  // CSS keyframe (which always starts from translateY(0) — snapping the sheet back
  // up first if a drag had already carried it most of the way down, then sliding
  // it back down again), this continues smoothly from wherever the sheet already
  // is (0 for a tap/select-triggered close, or the dragged position for a swipe).
  useEffect(() => {
    if (!closing) return;
    const el = sheetInnerRef.current;
    if (!el) return;
    const startingTransform = el.style.transform || "translateY(0px)";
    el.style.transition = "none";
    el.style.transform = startingTransform;
    let raf1, raf2;
  ...
  const onHandleTouchStart = e => {
    ...
    dragDelta.current = 0;
    if (sheetInnerRef.current) sheetInnerRef.current.style.transition = "none";
    // if the entrance animation hasn't finished yet, cut it short so our direct
    // transform edits below aren't fighting a still-active CSS animation.
    if (!entered) setEntered(true);
  };
  const onHandleTouchMove = e => {
    if (dragStartY.current == null) return;
    const dy = e.touches[0].clientY - dragStartY.current;
    // follows the finger both ways — downward moves freely, upward is rubber-banded
    // a bit (the sheet can't grow taller than its content) so it still feels alive.
    const translate = dy > 0 ? dy : dy * 0.3;
    dragDelta.current = dy;
    if (sheetInnerRef.current) sheetInnerRef.current.style.transform = `translateY(${translate}px)`;
  };
  const onHandleTouchEnd = () => {
    const shouldClose = dragDelta.current > 70;
    if (shouldClose) {
      // don't touch transform/transition here — the closing effect above picks up
      // exactly where the drag left off and continues the motion smoothly.
      requestClose();
    } else if (sheetInnerRef.current) {
      sheetInnerRef.current.style.transition = "transform 240ms cubic-bezier(0.22,1,0.36,1)";
      sheetInnerRef.current.style.transform = "";
    }
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      animation: `${closing ? "sheetBackdropOut" : "sheetBackdropIn"} 220ms ease forwards`
    }
    ...
    style: {
      ...
      // Once the enter animation finishes we drop it entirely — a CSS animation
      // with fill-mode:forwards otherwise keeps winning the cascade over any
      // transform we set directly via JS for the drag gesture, so dragging would
      // silently do nothing after the sheet had already slid up. Closing motion
      // is handled imperatively above, not via a keyframe, so it can continue
      // from the sheet's current (possibly dragged) position.
      animation: entered ? "none" : "sheetSlideUp 280ms cubic-bezier(0.22,1,0.36,1) forwards"
    }
  }, /*#__PURE__*/React.createElement("div", {
...
// ---------- Collapse: animates both opening AND closing of a section ----------
// A plain `{open && <div>...}` only animates the mount (open), since React
// unmounts the div instantly on close with nothing left to animate. This
// measures the real content height and transitions it (plus opacity), so
// the panel visibly grows/shrinks — not just its content fading in place.
function Collapse({
  open,
  children,
  duration = 260
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      // a new row while open) isn't clipped by a stale fixed max-height.
      timer = setTimeout(() => setMaxH("none"), duration + 40);
    } else {
...
function PickerSheet({
  ...
  }, title), !hideNoAssign && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      onSelect(null);
...
function ShiftCard({
  ...
  return /*#__PURE__*/React.createElement("div", {
    style: {
      background: isPast ? C.panelAlt : C.panel,
      border: `1px solid ${isActive ? C.amber : isPast ? C.border : C.border}`,
      borderRadius: 14,
      padding: 16,
      marginBottom: 14,
      position: "relative",
    ...
    style: {
      ...
      fontSize: 17,
      fontWeight: 800,
      color: isActive ? C.amber : isPast ? C.textMute : C.text,
      display: "flex",
      alignItems: "center",
      gap: 8
    }
    ...
    noteKinds: noteKinds,
    lastTower: lastTower
  })), canHaveCar && /*#__PURE__*/React.createElement("button", {
    type: "button",
    role: "switch",
    "aria-checked": !!data.carEnabled,
    onClick: () => onUpdate({
      ...data,
      carEnabled: !data.carEnabled,
      car: !data.carEnabled ? data.car : null
    }),
    style: {
      display: "flex",
      justifyContent: "space-between",
  ...
  }, "خدمة السيارة ", data.carEnabled ? "مفعّلة" : "غير مفعّلة"), /*#__PURE__*/React.createElement("span", {
    style: {
      width: 42,
      height: 24,
      borderRadius: 999,
      position: "relative",
      flexShrink: 0,
      background: data.carEnabled ? C.olive : C.borderLight,
    ...
    style: {
      ...
      left: data.carEnabled ? 20 : 2,
      boxShadow: "0 1px 3px rgba(0,0,0,0.35)"
    }
  }))), canHaveCar && data.carEnabled && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 8
    }
...
function SoldiersPanel({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, open ? "−" : "+")), /*#__PURE__*/React.createElement(Collapse, {
    open: open
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 14
    ...
    onPickPayment: type => onAddPaymentLeave(leaveSheetFor, type),
    onConfirmExceptional: (startDate, days) => onAddExceptionalLeave(leaveSheetFor, startDate, days)
  }));
}
...
function AddExceptionalFlow({
  ...
  // Soldiers already on leave (payment or another exceptional leave) today aren't
  // eligible for a new exceptional leave — they're already accounted for.
  const eligibleSoldiers = soldiers.filter(s => !isOnLeave(s, dateKey(new Date())));
  if (step === "pick") {
...
function LeavesPanel({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, MONTH_NAMES_SHORT[month], " ", year), !isCurrentMonth && /*#__PURE__*/React.createElement("button", {
  ...
  }, "الرجوع للشهر الحالي")), /*#__PURE__*/React.createElement("button", {
    onClick: () => changeMonth(1),
    style: navBtnStyle
  }, "›")), PAYMENT_TYPES.map(p => {
    const {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, sectionOpen ? "−" : "+"))), /*#__PURE__*/React.createElement(Collapse, {
      open: sectionOpen
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 10
      ...
      options: eligibleSoldiers,
      selectedId: null,
      onSelect: id => onAddPaymentLeave(id, p.type),
  ...
  }, openSections.exceptional ? "−" : "+")), /*#__PURE__*/React.createElement(Collapse, {
    open: openSections.exceptional
  }, /*#__PURE__*/React.createElement("div", null, exceptionalEntries.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      color: C.textFaint,
      fontFamily: FONT_BODY,
      fontSize: 13,
    ...
    onConfirm: (soldierId, startDate, days) => onAddExceptionalLeave(soldierId, startDate, days)
  })))));
}
...
function DutyRosterApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const loaded = res && res.value ? JSON.parse(res.value) : [];
        setSoldiers(pruneExpiredExceptionalLeaves(loaded, dateKey(new Date())));
      } catch {
        setSoldiers([]);
      }
  ...
  // Which (date, shiftId) is actually running right now. Night shifts start at 23:00
  // and run past midnight into the next calendar day, so between 00:00-07:00 the
  // "active" night shift still belongs to YESTERDAY's date, not today's.
  const activeShift = (() => {
    const h = now.getHours() + now.getMinutes() / 60;
    if (h < 7) return {
      date: addDays(now, -1),
  ...
  const isViewingActiveDay = isSameDay(dateObj, activeShift.date);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    rel: "stylesheet"
        ...
        input[type="text"], input[type="date"] { transition: border-color 150ms ease; }
      `), /*#__PURE__*/React.createElement("div", {
    style: {
      maxWidth: 520,
      margin: "0 auto",
    ...
    return /*#__PURE__*/React.createElement(ShiftCard, {
      ...
      isActive: isViewingActiveDay && activeShift.shiftId === shift.id,
      isPast: isPast,
