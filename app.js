...
function isOnLeave(soldier, dk) {
  return (soldier.leaves || []).some(l => dk >= l.startDate && dk <= l.endDate);
}
function activeLeaveFor(soldier, dk) {
  return (soldier.leaves || []).find(l => dk >= l.startDate && dk <= l.endDate) || null;
}
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      style: {
        ...
        marginBottom: 6,
        borderRadius: 10,
        border: `1px solid ${isSelected ? C.olive : note ? C.amber + "55" : C.border}`,
        background: isSelected ? C.oliveSoft : note ? C.amberSoft : "transparent",
...
function ShiftCard({
  ...
  soldiers.forEach(s => {
    ...
    if (justReturned && !hasWorkedEarlierToday(fullDayData, shift.id, s.id)) {
      if (shift.id === "morning") note = "رجع من إجازة";else if (shift.id === "afternoon") note = "رجع من إجازة + راحة ٨ ساعة";else note = "رجع من إجازة";
...
function LeavesPanel({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    const entries = soldiers.flatMap(s => (s.leaves || []).filter(l => l.type === p.type && l.paymentMonth === monthKeyStr).map(l => ({
      soldier: s,
      leave: l
    })));
    const eligibleSoldiers = soldiers.filter(s => !(s.leaves || []).some(l => PAYMENT_TYPES.some(pt => pt.type === l.type) && l.paymentMonth === monthKeyStr));
    return /*#__PURE__*/React.createElement("div", {
      key: p.type,
      style: {
        marginBottom: 18
    ...
    }, fmtDayMonth(start), " ← ", fmtDayMonth(end))), entries.map(({
      soldier,
      leave
      ...
      leave: leave,
      onDelta: d => onAdjustPaymentDays(soldier.id, leave.id, d),
      onDeductFrom: dir => onSetDeductFrom(soldier.id, leave.id, dir),
      ...
      selectedId: null,
      onSelect: id => onAddPaymentLeave(id, p.type, year, month),
  ...
  }, "إجازات استثنائية"), exceptionalEntries.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      color: C.textFaint,
      fontFamily: FONT_BODY,
      fontSize: 13,
...
function DutyRosterApp() {
  ...
  const addPaymentLeave = (soldierId, type, year, monthIndex) => {
    const now = new Date();
    const y = year != null ? year : now.getFullYear();
    const m = monthIndex != null ? monthIndex : now.getMonth();
    const anchor = PAYMENT_TYPES.find(p => p.type === type).anchorDay;
    const {
      start,
      end
    } = paymentWindow(y, m, anchor);
    const windowStart = dateKey(start);
    const windowEnd = dateKey(end);
    const paymentMonth = `${y}-${pad(m + 1)}`;
  ...
  const endLeaveNow = soldierId => {
    ...
    setSoldiers(prev => prev.map(s => {
      ...
      const leaves = (s.leaves || []).map(l => {
        const isActiveToday = todayK >= l.startDate && todayK <= l.endDate;
        if (!isActiveToday) return l;
  ...
  const adjustPaymentDays = (soldierId, leaveId, delta) => {
    setSoldiers(prev => prev.map(s => {
      if (s.id !== soldierId) return s;
      const leaves = (s.leaves || []).map(l => {
        ...
        const {
          startDate,
          endDate
        } = computeActualRange(l.windowStart, l.windowEnd, newDays, l.deductFrom);
        return {
          ...l,
          days: newDays,
          startDate,
  ...
  const setLeaveDeductFrom = (soldierId, leaveId, deductFrom) => {
    setSoldiers(prev => prev.map(s => {
      if (s.id !== soldierId) return s;
      const leaves = (s.leaves || []).map(l => {
        if (l.id !== leaveId) return l;
        const {
          startDate,
          endDate
        } = computeActualRange(l.windowStart, l.windowEnd, l.days, deductFrom);
        return {
          ...l,
          deductFrom,
          startDate,
